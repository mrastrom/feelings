(function ($) {
    ...
    class Boll { //Base class
        ...
        separationBehavior(magnitude) {
            ...
            let distance = 0;
            let counter = 0;
            //counter += doSeparationBehavior(this, angerFeelings, 5, steer);
            for (let i = 0; i < angerFeelings.length; i++) {
                distance = this.mesh.position.distanceTo(angerFeelings[i].mesh.position);
            ...
            //counter += doSeparationBehavior(this, joyFeels, 10, steer);
            ...
            //counter += doSeparationBehavior(this, fearFeels, 15, steer);
            ...
            // counter += doSeparationBehavior(this, generalFeels, 8, steer);
            ...
            // counter += doSeparationBehavior(this, loveFeels, 15, steer);
            ...
            //counter += doSeparationBehavior(this, sadnesFeelings, 50, steer);
    ...
    class JoyFeeling extends Boll {
        ...
        allGather() {
            if (gatherAll) {
                for (var i = 0; i < joyFeels.length; i++) { //walk through GeneralFeelings
  ...
  * Fear Feeling class
   */
    class FearFeeling extends Boll {
        constructor(feelData, font) {
        ...
        allGather() {
            if (gatherAll) {
                for (var i = 0; i < fearFeels.length; i++) { //walk through GeneralFeelings
    ...
    // angerFeelings
    class AngerFeeling extends Boll {
        constructor(feelData, font) {
        ...
        flock() {
            ...
            //arga bollar har hög separation och ganska hög cohesion för att de vill stångas med andra bollarg
    ...
    * Love Feeling class
     */
    class LoveFeeling extends Boll {
        constructor(feelData, font) {
    ...
    * General Feeling class
     */
    class GeneralFeeling extends Boll {
        constructor(feelData, font) {
    ...
    class SadnesFeeling extends Boll {
        constructor(feelData, font) {
    ...
    *   - Fix random function....
    *   - Fix feelings get caught under the mouse cursor.
     */
    ...
    function init() {
        ...
        console.log("maxwith:" + maxwidth + " maxhight:" + maxheight);
        console.log("Left:" + renderarea.offsetLeft + "Top:" + renderarea.offsetTop + " h:" + renderarea.offsetHeight + " w:" + renderarea.offsetWidth);
        initPlanes();

        //renderer = new THREE.WebGLRenderer({alpha: true});
        renderer = new THREE.WebGLRenderer();
        //renderer = new THREE.CanvasRenderer({canvas: renderarea,alpha: true});
    ...
    function makeVisualObjects(data) {
        for (index in data) {
            switch (data[index].type) {
                ...
                case "2": //Joy
                    feelData = new FeelingPresentationData(data[index].tweetText,data[index].tag);//tweetText, text, type, num
                        i = joyFeels.push(new JoyFeeling(feelData, font));
    ...
    /*
        function fillWithTestData() {
            ...
            //General
            for (var i = 0; i < 4; i++) {
                generalFeels.push(new GeneralFeeling());
                generalFeels[i].mesh.position.add(new THREE.Vector3(THREE.Math.randFloatSpread(maxwidth), THREE.Math.randFloatSpread(maxheight), 0));
                scene.add(generalFeels[i].mesh);
                //console.log("x:" + generalFeels[i].mesh.position.x + " y:" + generalFeels[i].mesh.position.y + " z:" + generalFeels[i].mesh.position.z);
            }
            //Love
            for (var i = 0; i < 10; i++) {
                feelData = new FeelingPresentationData("Love #" + i);
                loveFeels[i] = new LoveFeeling(feelData);
                loveFeels[i].mesh.position.add(new THREE.Vector3(THREE.Math.randFloatSpread(maxwidth), THREE.Math.randFloatSpread(maxheight), 0));
                scene.add(loveFeels[i].mesh);
            }
            //Fear
            for (var i = 0; i < 2; i++) {
                feelData = new FeelingPresentationData("Fear #" + i);
                fearFeels[i] = new FearFeeling(feelData);
                fearFeels[i].mesh.position.add(new THREE.Vector3(THREE.Math.randFloatSpread(maxwidth), THREE.Math.randFloatSpread(maxheight), 0));
                scene.add(fearFeels[i].mesh);
            }
            //Joy
            for (var i = 0; i < 10; i++) {
                feelData = new FeelingPresentationData("Joy #" + i);
                joyFeels[i] = new JoyFeeling(feelData);
                joyFeels[i].mesh.position.add(new THREE.Vector3(THREE.Math.randFloatSpread(maxwidth), THREE.Math.randFloatSpread(maxheight), 0));
                scene.add(joyFeels[i].mesh);
            }
            //SadnesFeeling
            for (var i = 0; i < 4; i++) {
                feelData = new FeelingPresentationData("Sadnes #" + i);
                sadnesFeelings[i] = new SadnesFeeling(feelData);
                sadnesFeelings[i].mesh.position.add(new THREE.Vector3(THREE.Math.randFloatSpread(maxwidth), THREE.Math.randFloatSpread(maxheight), 0));
                scene.add(sadnesFeelings[i].mesh);
            }
            //AngerFeeling
            for (var i = 0; i < 4; i++) {
                feelData = new FeelingPresentationData("Angry #" + i);
                angerFeelings[i] = new AngerFeeling(feelData);
                angerFeelings[i].mesh.position.add(new THREE.Vector3(THREE.Math.randFloatSpread(maxwidth), THREE.Math.randFloatSpread(maxheight), 0));
                scene.add(angerFeelings[i].mesh);
            }
    ...
    function updateMousePosition(event) {
        ...
        var pMouse = new THREE.Vector3((event.clientX / renderer.domElement.clientWidth) * 2.0 - 1.025,
            -(event.clientY / renderer.domElement.clientHeight) * 2.0 + 1.025,
            0);
        pMouse.unproject(camera);
        var m = pMouse.z / (pMouse.z - camera.position.z);
    ...
    function animate() {
        ...
        if (animating) {
            ...
            //window.animationId = requestAnimationFrame(animate);
            for (var i = 0; i < generalFeels.length; i++) {
                generalFeels[i].runIt();
            }
            for (var i = 0; i < loveFeels.length; i++) {
                loveFeels[i].runIt();
            }
            for (var i = 0; i < fearFeels.length; i++) {
                fearFeels[i].runIt();
            }
            for (var i = 0; i < joyFeels.length; i++) {
                joyFeels[i].runIt();
            }
            for (var i = 0; i < sadnesFeelings.length; i++) {
                sadnesFeelings[i].runIt();
            }
            for (var i = 0; i < angerFeelings.length; i++) {
    ...
    /*    function doSeparationBehavior(self, feelingList, distance, steer) {
            let counter = 0;
            let aDistance;
            for (let i = 0; i < feelingList.length; i++) {
                aDistance = self.mesh.position.distanceTo(feelingList[i].mesh.position);
                if (self.mesh.position.z > 0) {
                    console.log("floating in z-plane z:" + self.mesh.position.z);
                }
                if (aDistance > 0 && aDistance < distance) {
                    let diff = new THREE.Vector3();
                    diff.subVectors(self.mesh.position, feelingList[i].mesh.position);
                    diff.normalize();
                    diff.divideScalar(aDistance);
                    steer.add(diff);
                    counter++;
                }
            }
    ...
    */
    ...
    function TextBox(FeelingPresentationData) {
    ...
    function initPlane(planeLoc) {
        ...
        let material = new THREE.MeshLambertMaterial({color: 0xeff2f4, opacity: planeStartOpacity, transparent: false});
